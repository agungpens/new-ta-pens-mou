<script>
    let Kegiatan = {
    add: () => {
        window.location.href = `{{ route('kegiatan/add') }}`;
    },
    ubah: (elm) => {
        let data_id = $(elm).attr("data_id");
        window.location.href = `{{ route('kegiatan') }}` + "/ubah?id=" + data_id;
    },
    detail: (elm) => {
        let data_id = $(elm).attr("data_id");
        window.location.href = `{{ route('kegiatan') }}` + "/detail?id=" + data_id;
    },
    back: () => {
    ...
    deleteDataLampiranConfirm: (elm, id) => {
        ...
        $.ajax({
            ...
            dataType: "json",
            data: params,
            url:`{{ route('kegiatan/deleteDataLampiran') }}`,
    ...
    execUploadFile: (files, component) => {
        ...
        $.ajax({
            ...
            contentType: false,
            cache: false,
            url: `{{ route('kegiatan/execUploadFile') }}`,
    ...
    showDataMou: (elm) => {
        ...
        $.ajax({
            ...
            dataType: "html",
            data: params,
            url: `{{ route('kegiatan/showDataMou') }}`,
    ...
    showDataMoa: (elm, id_text) => {
        ...
        $.ajax({
            ...
            dataType: "html",
            data: params,
            url: `{{ route('kegiatan/showDataMoa') }}`,
    ...
    changeInstansi : (elm,e) => {
        // ambil data dari select yang di pilih
        let instansi = $(elm).val();
        $.ajax({
            type: "POST",
            url: `{{ route('kegiatan/searchDataByInstansi') }}`,
            data: {
                instansi: instansi
            },
            dataType: "json",
