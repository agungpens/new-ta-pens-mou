<script>
    ...
    add: () => {
        window.location.href = `{{ route('master-template-doc/add') }}`;
    },
    ubah: (elm) => {
        let data_id = $(elm).attr("data_id");
        window.location.href =`{{ route('master-template-doc') }}`+ "/ubah?id=" + data_id;
    },
    back: () => {
        window.location.href =`{{ route('master-template-doc') }}`;
    },
    ...
    deleteConfirm: (elm, id) => {
        ...
        $.ajax({
            ...
            dataType: 'json',
            data: params,
            url: `{{ route('master-template-doc/delete') }}`,
    ...
    submit: (elm, e) => {
        ...
        if (validation.runWithElement(form)) {
            $.ajax({
                ...
                dataType: 'json',
                data: params,
                url: `{{ route('master-template-doc/submit') }}`,
    ...
    getData: async () => {
        ...
        if (tableData.length > 0) {
            tableData.DataTable({
                ...
                "ajax": {
                    "url": `{{ route('master-template-doc/getData') }}`,
                    "type": "GET",
                    // "headers": {
