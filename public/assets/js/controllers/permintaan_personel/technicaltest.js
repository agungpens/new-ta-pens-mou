// var tableTechnicalTest ;

let TechnicalTest = {
    module: () => {
        return "karyawan/technicaltest";
    },

    moduleApi: () => {
        return `api/${TechnicalTest.module()}`;
    },

    add: () => {
        window.location.href = url.base_url(TechnicalTest.module()) + "add";
    },

    ubah: (elm) => {
        let data_id = $(elm).attr("data_id");
        window.location.href = url.base_url(TechnicalTest.module()) + "ubah?id=" + data_id;
    },

    approve: (elm) => {
        let data_id = $(elm).attr("data_id");
        window.location.href = url.base_url(TechnicalTest.module()) + "approve?id=" + data_id;
    },

    back: () => {
        window.location.href = url.base_url(TechnicalTest.module()) + "index";
    },
    ...
    getData: async () => {
        ...
        if (tableData.length > 0) {
            tableData.DataTable({
                ...
                "ajax": {
                    "url": url.base_url(TechnicalTest.moduleApi()) + `getData`,
                    "type": "POST",
                    // "headers": {
                ...
                "columns": [
                    ...
                    },
                    {
                        ...
                        "render": (data, type, row, meta) => {
                            ...
                            // if (approveAction == '1') {
                            // }
                            htmlAction += `<i class="bx bx-edit" style="cursor: pointer;" data_id="${data}" onclick="TechnicalTest.ubah(this)"></i>`;
                            // htmlAction += `<i class="bx bx-trash" style="cursor: pointer;" data_id="${data}" onclick="TechnicalTest.delete(this, event)"></i>`;
    ...
    filter: () => {
        ...
        // $('table#table-TechnicalTest').DataTable().destroy();
        // TechnicalTest.getDataTechnicalTest(urut1,urut2,urut3,urut1dir,urut2dir,urut3dir)
        $('#table-TechnicalTest').DataTable().ajax.reload();
    },

    getDataTechnicalTest: async (urut1,urut2,urut3,urut1dir,urut2dir,urut3dir) => {
        console.log(urut1,urut2,urut3,urut1dir,urut2dir,urut3dir);
        // tableTechnicalTest = $('table#table-TechnicalTest');
        console.log('#id',$('#id').val());

        // if (tableTechnicalTest.length > 0) {
            tableTechnicalTest = $('#table-TechnicalTest').DataTable({
                "processing": true,
                "serverSide": true,
                "ordering": true,
                "autoWidth": false,
                ...
                "ajax": {
                    "url": url.base_url(TechnicalTest.moduleApi()) + `showDetails`,
                    "type": "POST",
                    "data":function(d){
                        d.id = $('#id').val();
                ...
                "columns": [
                    ...
                    },
                    {
                        ...
                        "render": (data, type, row, meta) => {
                            ...
                            <input class="form-check-input actioncheckbox" style="float:none" type="checkbox" value="${row.id}" ${checked}>
                            </div>
                            `;
    ...
    delete: (elm, e) => {
        ...
        <div class="col-12 text-center">
            <br/>
            <button class="btn btn-danger btn-sm" onclick="TechnicalTest.deleteConfirm(this, '${data_id}')">Ya</button>
    ...
    deleteConfirm: (elm, id) => {
        ...
        $.ajax({
            ...
            dataType: 'json',
            data: params,
            url: url.base_url(TechnicalTest.moduleApi()) + "delete",
    ...
    submit: (elm, e) => {
        e.preventDefault();
        let params = TechnicalTest.getPostData();
        console.log(params);
        // return params;
        ...
        // if (validation.runWithElement(form)) {
            ...
            $.ajax({
                type: 'POST',
                dataType: 'json',
                data: TechnicalTest.getPostData(),
                url: url.base_url(TechnicalTest.moduleApi()) + "submit",
                beforeSend: () => {
    ...
    reset:() => {
        ...
        $('#urut3dir').val('').trigger('change');
        // $('table#table-TechnicalTest').DataTable().destroy();
        // TechnicalTest.getDataTechnicalTest(null,null,null,null,null,null)
        $('#table-TechnicalTest').DataTable().ajax.reload();
    },
...
$(function () {
    TechnicalTest.getData();
    // TechnicalTest.getDataTechnicalTest();
    // TechnicalTest.editor();
    // TechnicalTest.getStepRekrutmenData();
    // TechnicalTest.getDataLogKaryawan();
    TechnicalTest.setDate();
    TechnicalTest.select2All();
    // TechnicalTest.removeWarning();
});
