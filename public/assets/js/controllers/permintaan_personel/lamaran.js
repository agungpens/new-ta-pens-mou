let Lamaran = {
    module: () => {
        return "karyawan/lamaran";
    },

    moduleApi: () => {
        return `api/${Lamaran.module()}`;
    },

    add: () => {
        window.location.href = url.base_url(Lamaran.module()) + "add";
    },

    ubah: (elm) => {
        let data_id = $(elm).attr("data_id");
        window.location.href = url.base_url(Lamaran.module()) + "ubah?id=" + data_id;
    },

    approve: (elm) => {
        let data_id = $(elm).attr("data_id");
        window.location.href = url.base_url(Lamaran.module()) + "approve?id=" + data_id;
    },

    back: () => {
        window.location.href = url.base_url(Lamaran.module()) + "index";
    },
    ...
    getData: async () => {
        ...
        if (tableData.length > 0) {
            tableData.DataTable({
                ...
                "ajax": {
                    "url": url.base_url(Lamaran.moduleApi()) + `getData`,
                    "type": "POST",
                    // "headers": {
                ...
                "columns": [
                    ...
                    },
                    {
                        ...
                        "render": (data, type, row, meta) => {
                            ...
                            // if (approveAction == '1') {
                            // }
                            htmlAction += `<i class="bx bx-edit" style="cursor: pointer;" data_id="${data}" onclick="Lamaran.ubah(this)"></i>`;
                            // htmlAction += `<i class="bx bx-trash" style="cursor: pointer;" data_id="${data}" onclick="Lamaran.delete(this, event)"></i>`;
    ...
    filter: () => {
        ...
        // $('table#table-lamaran').DataTable().destroy();
        // Lamaran.getDataLamaran(urut1,urut2,urut3,urut1dir,urut2dir,urut3dir)
        $('#table-lamaran').DataTable().ajax.reload();
    },

    getDataLamaran: async (urut1,urut2,urut3,urut1dir,urut2dir,urut3dir) => {
        console.log(urut1,urut2,urut3,urut1dir,urut2dir,urut3dir);
        // tableLamaran = $('table#table-lamaran');
        console.log('#id',$('#id').val());

        // if (tableLamaran.length > 0) {
            tableLamaran = $('#table-lamaran').DataTable({
                "processing": true,
                "serverSide": true,
                "ordering": true,
                "autoWidth": false,
                ...
                "ajax": {
                    "url": url.base_url(Lamaran.moduleApi()) + `showDetails`,
                    "type": "POST",
                    "data":function(d){
                        d.id = $('#id').val();
    ...
    delete: (elm, e) => {
        ...
        <div class="col-12 text-center">
            <br/>
            <button class="btn btn-danger btn-sm" onclick="Lamaran.deleteConfirm(this, '${data_id}')">Ya</button>
    ...
    deleteConfirm: (elm, id) => {
        ...
        $.ajax({
            ...
            dataType: 'json',
            data: params,
            url: url.base_url(Lamaran.moduleApi()) + "delete",
    ...
    submit: (elm, e) => {
        e.preventDefault();
        let params = Lamaran.getPostData();
        console.log(params);
        // return params;
        ...
        // if (validation.runWithElement(form)) {
            ...
            $.ajax({
                type: 'POST',
                dataType: 'json',
                data: Lamaran.getPostData(),
                url: url.base_url(Lamaran.moduleApi()) + "submit",
                beforeSend: () => {
    ...
    reset:() => {
        ...
        $('#urut3dir').val('').trigger('change');
        // $('table#table-lamaran').DataTable().destroy();
        // Lamaran.getDataLamaran(null,null,null,null,null,null)
        $('#table-lamaran').DataTable().ajax.reload();
    },
...
$(function () {
    Lamaran.getData();
    Lamaran.getDataLamaran();
    // Lamaran.editor();
    // Lamaran.getStepRekrutmenData();
    // Lamaran.getDataLogKaryawan();
    Lamaran.setDate();
    Lamaran.select2All();
    // Lamaran.removeWarning();
});
