let Menu = {
    ...
    ubah: (elm) => {
        ...
        window.location.href = url.base_url(Menu.module()) + "ubah?id=" + data_id;
    },

    back: () => {
        window.location.href = url.base_url(Menu.module()) + "index";
    },
                ...
                "columns": [{
                    ...
                    },
                    {
                        ...
                        "render": (data, type, row, meta) => {
                            let htmlAction = '';
                            if(updateAction == '1'){
                                htmlAction += `<i class="bx bx-edit" style="cursor: pointer;" data_id="${data}" onclick="Menu.ubah(this)"></i>`;
                            }                            
                            
                            if(deleteAction == '1'){
                                htmlAction += `<i class="bx bx-trash" style="cursor: pointer;" data_id="${data}" onclick="Menu.delete(this, event)"></i>`;
                            }                            
                            return htmlAction;
                        }
                    }
    ...
    delete: (elm, e) => {
        ...
        <div class="col-12 text-center">
            <br/>
            <button class="btn btn-primary btn-sm" onclick="Menu.deleteConfirm(this, '${data_id}')">Ya</button>
    ...
    getPostData: () => {
        let data = {
            'data': {
                'id': $('input#id').val(),
                'parent_menu': $.trim($('#parent-menu').val()),
                'menu': $.trim($('#menu').val()),
                'menu_url': $.trim($('#menu_url').val()),
                'menu_url_mobile': $.trim($('#menu_url_mobile').val()),
            },
        };
        return data;
    },
    ...
    getDataLogKaryawan: async () => {
        ...
        if (tableData.length > 0) {
            tableData.DataTable({
                ...
                "ajax": {
                    "url": url.base_url(Menu.moduleApi()) + `getDataLogKaryawan`,
                    "type": "GET",
                    "data": params
                    // "headers": {
                    //     'X-CSRF-TOKEN': `'${tokenApi}'`
                ...
                "columns": [{
                    ...
                    },
                    {
                        "data": "doc_trans",
                        "render": (data, type, row, meta) => {
                            return `<a class="" style="cursor: pointer;" onclick="Menu.showDetailEditProfile(this, event)">${data}</a>`;
                        }
                    },
                    {
                        "data": "tgl_pengajuan",
                    ...
                    },
                    {
                        ...
                        "render": (data, type, row, meta) => {
                            return `
                            <i class="bx bx-edit" style="cursor: pointer;" data_id="${data}" onclick="Menu.detailPerubahan(this)"></i>`;
                        }
                    }
                ]
            });
    ...
    modulePerubahanApi: () => {
        return `api/${Menu.modulePerubahan()}`;
    },

    detailPerubahan: (elm) => {
        let data_id = $(elm).attr("data_id");
        let from_id = $('input#id').val();
        window.location.href = url.base_url(Menu.modulePerubahan()) + "ubah?id=" + data_id + "&state=karyawan-" + from_id;
    },
    ...
    showDetailEditProfile:(elm, e)=>{
        ...
        $.ajax({
            ...
            dataType: 'html',
            data: params,
            url: url.base_url(Menu.modulePerubahanApi()) + "showDetailEditProfile",
