let News = {
    ...
    moduleApi: () => {
        return `api/${News.module()}`;
    },

    add: () => {
        window.location.href = url.base_url(News.module()) + "add";
    },

    ubah: (elm) => {
        let data_id = $(elm).attr("data_id");
        window.location.href = url.base_url(News.module()) + "ubah?id=" + data_id;
    },

    back: () => {
        window.location.href = url.base_url(News.module()) + "index";
    },
    ...
    getData: async () => {
        ...
        if (tableData.length > 0) {
            ...
            tableData.DataTable({
                ...
                "ajax": {
                    "url": url.base_url(News.moduleApi()) + `getData`,
                    "type": "GET",
                    // "headers": {
                ...
                "columns": [{
                    ...
                    },
                    {
                        ...
                        "render": (data, type, row, meta) => {
                            let htmlAction = '';
                            if (updateAction == '1') {
                                htmlAction += `<i class="bx bx-edit" style="cursor: pointer;" data_id="${data}" onclick="News.ubah(this)"></i>`;
                            }

                            if (deleteAction == '1') {
                                htmlAction += `<i class="bx bx-trash" style="cursor: pointer;" data_id="${data}" onclick="News.delete(this, event)"></i>`;
                            }
                            return htmlAction;
                        }
                    }
    ...
    delete: (elm, e) => {
        ...
        <div class="col-12 text-center">
            <br/>
            <button class="btn btn-primary btn-sm" onclick="News.deleteConfirm(this, '${data_id}')">Ya</button>
    ...
    deleteConfirm: (elm, id) => {
        ...
        $.ajax({
            ...
            dataType: 'json',
            data: params,
            url: url.base_url(News.moduleApi()) + "delete",
    ...
    submit: (elm, e) => {
        ...
        if (validation.runWithElement(form)) {
            $.ajax({
                ...
                dataType: 'json',
                data: params,
                url: url.base_url(News.moduleApi()) + "submit",
                beforeSend: () => {
        ...
        uploader.on("change", function () {
            ...
            reader.onload = function (event) {
                ...
                if (type_file == 'jpg' || type_file == 'jpeg' || type_file == 'png') {
                    ...
                    News.execUploadFile(files, src_foto);
        ...
        uploader.on("change", function () {
            ...
            reader.onload = function (event) {
                ...
                if (type_file == 'pdf') {
                    ...
                    News.execUploadFile(files, src_file);
    ...
    execUploadFile: (files, component) => {
        ...
        $.ajax({
            ...
            contentType: false,
            cache: false,
            url: url.base_url(News.moduleApi()) + "execUploadFile",
    ...
    getDataLogKaryawan: async () => {
        ...
        if (tableData.length > 0) {
            tableData.DataTable({
                ...
                "ajax": {
                    "url": url.base_url(News.moduleApi()) + `getDataLogKaryawan`,
                    "type": "GET",
                    "data": params
                    // "headers": {
                    //     'X-CSRF-TOKEN': `'${tokenApi}'`
                ...
                "columns": [{
                    ...
                    },
                    {
                        "data": "doc_trans",
                        "render": (data, type, row, meta) => {
                            return `<a class="" style="cursor: pointer;" onclick="News.showDetailEditProfile(this, event)">${data}</a>`;
                        }
                    },
                    {
                        "data": "tgl_pengajuan",
                    ...
                    },
                    {
                        ...
                        "render": (data, type, row, meta) => {
                            return `
                            <i class="bx bx-edit" style="cursor: pointer;" data_id="${data}" onclick="News.detailPerubahan(this)"></i>`;
                        }
                    }
                ]
            });
    ...
    modulePerubahanApi: () => {
        return `api/${News.modulePerubahan()}`;
    },

    detailPerubahan: (elm) => {
        let data_id = $(elm).attr("data_id");
        let from_id = $('input#id').val();
        window.location.href = url.base_url(News.modulePerubahan()) + "ubah?id=" + data_id + "&state=karyawan-" + from_id;
    },
    ...
    showDetailEditProfile: (elm, e) => {
        ...
        $.ajax({
            ...
            dataType: 'html',
            data: params,
            url: url.base_url(News.modulePerubahanApi()) + "showDetailEditProfile",
