let Divisi = {
    ...
    moduleApi: () => {
        return `api/${Divisi.module()}`;
    },

    add: () => {
        window.location.href = url.base_url(Divisi.module()) + "add";
    },

    ubah: (elm) => {
        let data_id = $(elm).attr("data_id");
        window.location.href = url.base_url(Divisi.module()) + "ubah?id=" + data_id;
    },

    back: () => {
        window.location.href = url.base_url(Divisi.module()) + "index";
    },
    ...
    getData: async () => {
        ...
        if (tableData.length > 0) {
            ...
            tableData.DataTable({
                ...
                "ajax": {
                    "url": url.base_url(Divisi.moduleApi()) + `getData`,
                    "type": "GET",
                    // "headers": {
                ...
                "columns": [{
                    ...
                    },
                    {
                        ...
                        "render": (data, type, row, meta) => {
                            let htmlAction = '';
                            if(updateAction == '1'){
                                htmlAction += `<i class="bx bx-edit" style="cursor: pointer;" data_id="${data}" onclick="Divisi.ubah(this)"></i>`;
                            }                            
                            
                            if(deleteAction == '1'){
                                htmlAction += `<i class="bx bx-trash" style="cursor: pointer;" data_id="${data}" onclick="Divisi.delete(this, event)"></i>`;
                            }                            
                            return htmlAction;
                        }
                    }
    ...
    delete: (elm, e) => {
        ...
        <div class="col-12 text-center">
            <br/>
            <button class="btn btn-primary btn-sm" onclick="Divisi.deleteConfirm(this, '${data_id}')">Ya</button>
    ...
    deleteConfirm: (elm, id) => {
        ...
        $.ajax({
            ...
            dataType: 'json',
            data: params,
            url: url.base_url(Divisi.moduleApi()) + "delete",
    ...
    submit: (elm, e) => {
        ...
        if(validation.runWithElement(form)){
            $.ajax({
                ...
                dataType: 'json',
                data: params,
                url: url.base_url(Divisi.moduleApi()) + "submit",
                beforeSend: () => {
    ...
    getDataLogKaryawan: async () => {
        ...
        if (tableData.length > 0) {
            tableData.DataTable({
                ...
                "ajax": {
                    "url": url.base_url(Divisi.moduleApi()) + `getDataLogKaryawan`,
                    "type": "GET",
                    "data": params
                    // "headers": {
                    //     'X-CSRF-TOKEN': `'${tokenApi}'`
                ...
                "columns": [{
                    ...
                    },
                    {
                        "data": "doc_trans",
                        "render": (data, type, row, meta) => {
                            return `<a class="" style="cursor: pointer;" onclick="Divisi.showDetailEditProfile(this, event)">${data}</a>`;
                        }
                    },
                    {
                        "data": "tgl_pengajuan",
                    ...
                    },
                    {
                        ...
                        "render": (data, type, row, meta) => {
                            return `
                            <i class="bx bx-edit" style="cursor: pointer;" data_id="${data}" onclick="Divisi.detailPerubahan(this)"></i>`;
                        }
                    }
                ]
            });
    ...
    modulePerubahanApi: () => {
        return `api/${Divisi.modulePerubahan()}`;
    },

    detailPerubahan: (elm) => {
        let data_id = $(elm).attr("data_id");
        let from_id = $('input#id').val();
        window.location.href = url.base_url(Divisi.modulePerubahan()) + "ubah?id=" + data_id + "&state=karyawan-" + from_id;
    },
    ...
    showDetailEditProfile:(elm, e)=>{
        ...
        $.ajax({
            ...
            dataType: 'html',
            data: params,
            url: url.base_url(Divisi.modulePerubahanApi()) + "showDetailEditProfile",
