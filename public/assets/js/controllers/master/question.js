let Pertanyaan = {
    module: () => {
        return "master/pertanyaan";
    },

    moduleApi: () => {
        return `api/${Pertanyaan.module()}`;
    },

    add: () => {
        window.location.href = url.base_url(Pertanyaan.module()) + "add";
    },

    ubah: (elm) => {
        let data_id = $(elm).attr("data_id");
        window.location.href = url.base_url(Pertanyaan.module()) + "ubah?id=" + data_id;
    },

    back: () => {
        window.location.href = url.base_url(Pertanyaan.module()) + "index";
    },
    ...
    getData: async () => {
        ...
        if (tableData.length > 0) {
            ...
            tableData.DataTable({
                ...
                "ajax": {
                    "url": url.base_url(Pertanyaan.moduleApi()) + `getData`,
                    "type": "POST",
                    // "headers": {
                ...
                "columns": [{
                    ...
                    // },
                    {
                        ...
                        "render": (data, type, row, meta) => {
                            let htmlAction = '';
                            if(updateAction == '1'){
                                htmlAction += `<i class="bx bx-edit" style="cursor: pointer;" data_id="${data}" onclick="Pertanyaan.ubah(this)"></i>`;
                            }                            
                            
                            if(deleteAction == '1'){
                                htmlAction += `<i class="bx bx-trash" style="cursor: pointer;" data_id="${data}" onclick="Pertanyaan.delete(this, event)"></i>`;
                            }                            
                            return htmlAction;
                        }
                    }
    ...
    delete: (elm, e) => {
        ...
        <div class="col-12 text-center">
            <br/>
            <button class="btn btn-primary btn-sm" onclick="Pertanyaan.deleteConfirm(this, '${data_id}')">Ya</button>
    ...
    deleteConfirm: (elm, id) => {
        ...
        $.ajax({
            ...
            dataType: 'json',
            data: params,
            url: url.base_url(Pertanyaan.moduleApi()) + "delete",
    ...
    submit: (elm, e) => {
        e.preventDefault();
        let params = Pertanyaan.getPostData();
        ...
        if(validation.runWithElement(form)){
            $.ajax({
                ...
                dataType: 'json',
                data: params,
                url: url.base_url(Pertanyaan.moduleApi()) + "submit",
                beforeSend: () => {
