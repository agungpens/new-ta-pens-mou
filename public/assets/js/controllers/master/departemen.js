let Departemen = {
    ...
    moduleApi: () => {
        return `api/${Departemen.module()}`;
    },

    add: () => {
        window.location.href = url.base_url(Departemen.module()) + "add";
    },

    ubah: (elm) => {
        let data_id = $(elm).attr("data_id");
        window.location.href = url.base_url(Departemen.module()) + "ubah?id=" + data_id;
    },

    back: () => {
        window.location.href = url.base_url(Departemen.module()) + "index";
    },
    ...
    getData: async () => {
        ...
        if (tableData.length > 0) {
            ...
            tableData.DataTable({
                ...
                "ajax": {
                    "url": url.base_url(Departemen.moduleApi()) + `getData`,
                    "type": "GET",
                    // "headers": {
                ...
                "columns": [{
                    ...
                    // },
                    {
                        ...
                        "render": (data, type, row, meta) => {
                            let htmlAction = '';
                            if(updateAction == '1'){
                                htmlAction += `<i class="bx bx-edit" style="cursor: pointer;" data_id="${data}" onclick="Departemen.ubah(this)"></i>`;
                            }                            
                            
                            if(deleteAction == '1'){
                                htmlAction += `<i class="bx bx-trash" style="cursor: pointer;" data_id="${data}" onclick="Departemen.delete(this, event)"></i>`;
                            }                            
                            return htmlAction;
                        }
                    }
    ...
    delete: (elm, e) => {
        ...
        <div class="col-12 text-center">
            <br/>
            <button class="btn btn-primary btn-sm" onclick="Departemen.deleteConfirm(this, '${data_id}')">Ya</button>
    ...
    deleteConfirm: (elm, id) => {
        ...
        $.ajax({
            ...
            dataType: 'json',
            data: params,
            url: url.base_url(Departemen.moduleApi()) + "delete",
    ...
    submit: (elm, e) => {
        e.preventDefault();
        let params = Departemen.getPostData();
        ...
        if(validation.runWithElement(form)){
            $.ajax({
                ...
                dataType: 'json',
                data: params,
                url: url.base_url(Departemen.moduleApi()) + "submit",
                beforeSend: () => {
    ...
    getDataLogKaryawan: async () => {
        ...
        if (tableData.length > 0) {
            tableData.DataTable({
                ...
                "ajax": {
                    "url": url.base_url(Departemen.moduleApi()) + `getDataLogKaryawan`,
                    "type": "GET",
                    "data": params
                    // "headers": {
                    //     'X-CSRF-TOKEN': `'${tokenApi}'`
                ...
                "columns": [{
                    ...
                    },
                    {
                        "data": "doc_trans",
                        "render": (data, type, row, meta) => {
                            return `<a class="" style="cursor: pointer;" onclick="Departemen.showDetailEditProfile(this, event)">${data}</a>`;
                        }
                    },
                    {
                        "data": "tgl_pengajuan",
                    ...
                    },
                    {
                        ...
                        "render": (data, type, row, meta) => {
                            return `
                            <i class="bx bx-edit" style="cursor: pointer;" data_id="${data}" onclick="Departemen.detailPerubahan(this)"></i>`;
                        }
                    }
                ]
            });
    ...
    modulePerubahanApi: () => {
        return `api/${Departemen.modulePerubahan()}`;
    },

    detailPerubahan: (elm) => {
        let data_id = $(elm).attr("data_id");
        let from_id = $('input#id').val();
        window.location.href = url.base_url(Departemen.modulePerubahan()) + "ubah?id=" + data_id + "&state=karyawan-" + from_id;
    },
    ...
    showDetailEditProfile:(elm, e)=>{
        ...
        $.ajax({
            ...
            dataType: 'html',
            data: params,
            url: url.base_url(Departemen.modulePerubahanApi()) + "showDetailEditProfile",
...
$(function () {
    Departemen.getData();
    Departemen.getDataLogKaryawan();
    Departemen.setDate();
    Departemen.select2All();
});
