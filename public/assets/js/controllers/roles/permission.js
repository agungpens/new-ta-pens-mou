...
let Permission = {
    ...
    addAll: () => {
        window.location.href = url.base_url(Permission.module()) + "addAll";
    },

    ubah: (elm) => {
        let data_id = $(elm).attr("data_id");
        window.location.href = url.base_url(Permission.module()) + "ubah?id=" + data_id;
    },

    back: () => {
        window.location.href = url.base_url(Permission.module()) + "index";
    },
                ...
                "columns": [{
                    ...
                    },
                    {
                        ...
                        "render": (data, type, row, meta) => {
                            let htmlAction = '';
                            if(updateAction == 1){
                                htmlAction += `<i class="bx bx-edit" style="cursor: pointer;" data_id="${data}" onclick="Permission.ubah(this)"></i>`;
                            }

                            if(deleteAction == 1){
                                htmlAction += `<i class="bx bx-trash" style="cursor: pointer;" data_id="${data}" onclick="Permission.delete(this, event)"></i>`;
                            }
                            return htmlAction;
                        }
                    }
    ...
    delete: (elm, e) => {
        ...
        <div class="col-12 text-center">
            <br/>
            <button class="btn btn-primary btn-sm" onclick="Permission.deleteConfirm(this, '${data_id}')">Ya</button>
    ...
    submitAll: (elm, e) => {
        e.preventDefault();
        let params = Permission.getPostDataAll();
        let form = $(elm).closest('div.row');
        if(validation.runWithElement(form)){
            $.ajax({
                type: 'POST',
                dataType: 'json',
                data: params,
                url: url.base_url(Permission.moduleApi()) + "submitAll",
                beforeSend: () => {
                    message.loadingProses('Proses Simpan Data...');
                },
                error: function () {
    ...
    getDataLogKaryawan: async () => {
        ...
        if (tableData.length > 0) {
            tableData.DataTable({
                ...
                "ajax": {
                    "url": url.base_url(Permission.moduleApi()) + `getDataLogKaryawan`,
                    "type": "GET",
                    "data": params
                    // "headers": {
                    //     'X-CSRF-TOKEN': `'${tokenApi}'`
                ...
                "columns": [{
                    ...
                    },
                    {
                        "data": "doc_trans",
                        "render": (data, type, row, meta) => {
                            return `<a class="" style="cursor: pointer;" onclick="Permission.showDetailEditProfile(this, event)">${data}</a>`;
                        }
                    },
                    {
                        "data": "tgl_pengajuan",
                    ...
                    },
                    {
                        ...
                        "render": (data, type, row, meta) => {
                            return `
                            <i class="bx bx-edit" style="cursor: pointer;" data_id="${data}" onclick="Permission.detailPerubahan(this)"></i>`;
                        }
                    }
                ]
            });
    ...
    modulePerubahanApi: () => {
        return `api/${Permission.modulePerubahan()}`;
    },

    detailPerubahan: (elm) => {
        let data_id = $(elm).attr("data_id");
        let from_id = $('input#id').val();
        window.location.href = url.base_url(Permission.modulePerubahan()) + "ubah?id=" + data_id + "&state=karyawan-" + from_id;
    },
    ...
    showDetailEditProfile:(elm, e)=>{
        ...
        $.ajax({
            ...
            dataType: 'html',
            data: params,
            url: url.base_url(Permission.modulePerubahanApi()) + "showDetailEditProfile",
...
$(function () {
    Permission.getData();
    Permission.getDataLogKaryawan();
    Permission.setDate();
    Permission.select2All();
    Permission.checkMenu();
});
